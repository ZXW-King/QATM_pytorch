...
def plot_result(image_raw, boxes, scores, show=False, save_name=None, color=(0, 0, 255)):
    # plot result
    d_img = image_raw.copy()
    for box, score in zip(boxes, scores):
        ...
        text_position = (int((box[0][0] + box[1][0] - text_size[0]) / 2), int(box[0][1] - 2))
        d_img = cv2.putText(d_img, text, text_position, font, font_scale, color, thickness, cv2.LINE_AA)
...
def nms_multi(scores, w_array, h_array, thresh_list):
    ...
    for index, score in zip(indices_omit, scores_omit):
        ...
        # 存储每个目标类别的得分分数
        scores_list.extend(score[dot[0], dot[1]])
    ...
    boxes = np.array([[x1[keep], y1[keep]], [x2[keep], y2[keep]]]).transpose(2, 0, 1)
    return boxes, np.array(keep_index), np.array(keep_scores)


def plot_result_multi(image_raw, boxes, indices, show=False, save_name=None, color_list=None, sco = None):
    d_img = image_raw.copy()
    if color_list is None:
        color_list = color_palette("deep", indices.max()+1)
        color_list = list(map(lambda x: (int(x[0]*255), int(x[1]*255), int(x[2]*255)), color_list))
    for i in range(len(indices)):
        d_img = plot_result(d_img, boxes[i][None, :,:].copy(),scores=sco, color=color_list[indices[i]])
    if show:
        plt.imshow(d_img[:,:,::-1])
    if save_name:
        cv2.imwrite(save_name, d_img)
